	    ...
	    $.fn.fn_blur = function(method){
	    	if(settings.blurBG){
		        ...
		        if(methods[method]){
		            ...
		            $.error('Method ' +  method + ' does not exist on jQuery.blur');
		        }
		    }
		    else{
		    	return false;
		...
		return this.each(function(){
			...
			var app = {
				...
				cacheElements: function(){
					...
					this.modal       = $('<div class="modal"></div>').css({'z-index': '999', 'border-radius': settings.modal_border_radius, 'box-shadow': settings.modal_box_shadow, 'padding': settings.modal_padding, 'border': settings.modal_border_style, 'background': settings.modal_background}).hide();
					this.new_image   = $('<img>');
