...
;(function($, window, document, undefined){
                    ...
                    if(this.errorArray.length > 0){
                        ...
                        $.each(this.errorArray, function(i){
                            ...
                            if(field.is('input[type="checkbox"]') || field.is('input[type="radio"]')){
                                ...
                                var valmessage = valfield.val();
                            }
                            else{
                                var labelField = field.parent().find('label');
                                var valfield   = field.parent().find('.' + settings.validationMessage);
                                var valmessage = valfield.text();
                            }
                            valmessage = (valmessage !== '') ? valmessage : settings.defaultErrorMsg;
    ...
    $(function(){
        $('.email, input[type="email"]').each(function(){
            $(this).after('<div id="suggestion">Did you mean <a id="alternate_email"><span class="toplevel">something</span>@<span class="dom">yourdomain.com</span></a>?</div>');
        });
    ...
    EMAIL.emailsuggest = {
        query: {
            init: function(){
                ...
                $('form').on('blur', '.email, input[type="email"]', this.suggestEmail);
                $('form').on('click', '#suggestion #alternate_email', this.fillInSuggestedEmail);
            },
            ...
            fillInSuggestedEmail: function(a){
